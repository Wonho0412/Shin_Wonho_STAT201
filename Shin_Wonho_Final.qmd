...
while (sum_val < 30) {
  ...
  print(rand_num)
  sum_val <- sum_val + rand_num
  count <- count +1
}

cat("You generated", count, "numbers that sum to", sum_val, ".\n")
...
result <- calc_bmi(66, 150)
bmi_value <- round(result$bmi, 1)
bmi_category <- result$category

cat("Your BMI is", bmi_category, "with a value of", bmi_value, ".\n")
```
...
```{r}
even_sum <- sapply(my_list, function(x) sum(x) %% 2 == 0)
print(even_sum)
```
...
for (i in 1:nrow(fruit_info)) {
  ...
  cat("The", fruit_info$fruit[i], "is", weight_class, ".\n")
}
```
