...
index <- which(spread == max_spread)

cat("The largest win spread is", max_spread, 
    ", when the Bears scored", bears[index], 
    "points and the Opponent scored", opponent[index], "points.\n")
```
...
```{r}
first_win_index <- which(game_score$win)[1]

if (!is.na(first_win_index) && first_win_index > 1) {
  losses_before_first_win <- sum(!game_score$win[1:(first_win_index - 1)])
} else {
...
area_rectangle <- function(length = 48, width = 48) {
  ...
  return(c(area_inches = area_inches, area_feet = area_feet))
}

area_result <- area_rectangle(102, 60)
cat("The area in square inches is", area_result["area_inches"], ".\n")
cat("The area in square feet is", area_result["area_feet"], ".\n")
```
...
```{r}
even_to_one_odd_to_zero <- function(mat) {
  mat[] <- ifelse(mat %% 2 == 0, 1, 0)
  return(mat)
}

even_to_one_odd_to_zero(x)
```
